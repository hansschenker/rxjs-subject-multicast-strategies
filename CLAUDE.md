...
A research/prototype project on RxJS Subject multicasting strategies. `docs/` is a wired-up VitePress site (config in `docs/.vitepress/config.mts`) documenting every strategy with a guide page per era, a migration guide, a Subject Router Algebra theory page (`docs/guide/router-algebra.md`, which embeds the spec invariants via a `#region invariants` marker in the test file — do not remove those markers), and an interactive Reset Flags Playground (`docs/guide/playground.md` + the `ResetFlagsPlayground` Vue component in `docs/.vitepress/theme/components/`, registered globally in `theme/index.ts`, driving a real rxjs `share()` pipeline); the raw research notes (NotebookLM exports, transcript) sit in the same folder but are excluded from the site via `srcExclude`. `src/samples/` holds one runnable, type-checked TypeScript sample per multicast strategy (run directly with `node src/samples/<file>.ts` — Node 24 executes erasable-syntax TS natively); `src/main.ts` is still the untouched Vite starter. Vitest is wired up: `npm test` runs `src/tests/share-reset-profiles.test.ts`, which pins all 8 `share()` reset profiles across idle/complete/error. Repository: https://github.com/hansschenker/rxjs-subject-multicast-strategies (commit directly to `main`). **Scope: RxJS 7 only** (`rxjs@^7.8.2`) — RxJS 8 may never be published and is out of scope; treat the deprecated-to-modern operator mapping as an intra-v7 modernization, never as a v8 migration.
...
- `npm test` — Vitest single run (`npm run test:watch` for watch mode); specs live in `src/tests/` and are type-checked by `npm run build`
...
1. **The 8 reset profiles** — the permutations of `share()`'s `[resetOnRefCountZero, resetOnComplete, resetOnError]` flags, graded by production utility: `[t,t,t]` default clean-slate broadcast (transient realtime data); `[f,f,t]` **Resilient Static Cache** — "cache success, retry failure", paired with `ReplaySubject(1)` (the headline production pattern); `[t,f,f]` terminal-state cache (the `t` matters only pre-terminal); `[f,f,f]` permanent application singleton; `[f,t,f]` the useless theoretical outlier (resets on success, permanently locks on failure).
...
`src/tests/share-reset-profiles.test.ts` proved two invariants that the NotebookLM research notes in `docs/` get wrong — trust the specs, not the notes, on these:

- `resetOnRefCountZero: false` keeps the upstream **connected** while idle (values keep flowing into the retained Subject — the shareReplay leak). It does not merely retain the Subject.
- After a terminal event, only the matching terminal flag (`resetOnComplete`/`resetOnError`) governs retention; refCount reaching zero can never evict a retained terminal Subject. Hence `[t,f,f]` is a permanent terminal cache, not a "GC-on-unmount" cache, and `[t,t,f]`'s error lock is permanent, not temporary.
